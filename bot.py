...
# 1 Displayed Coin = 1000 Internal Credits (Stored in DB)
# If you have 25000 in DB, it shows as 25,000 Credits.
...
def init_db():
    ...
    # Add default local node if missing
    ...
    # ==========================================
    # RESET & INITIALIZE PACKAGES
    # ==========================================
    # Clearing old plans to ensure clean state as requested
    cur.execute('DELETE FROM deploy_plans')
    cur.execute('DELETE FROM resource_plans')

    # New Packages (Based on User Request)
    new_deploy_plans = [
        # Original 7 Plans
        ...
        # The 2 Custom Plans requested at the end
    ...
    # Economy Settings (Scaled by CREDIT_MULTIPLIER)
    coin_settings = [
...
# ==========================================
# THEME & EMBED SYSTEM (RED ONLY)
# ==========================================

class EmbedColors:
    """RED Theme Color Palette"""
    PRIMARY = 0xFF0000      # Pure Red
    SUCCESS = 0xFF0000      # Red
    ERROR = 0x8B0000        # Dark Red
    WARNING = 0xFF0000      # Red
    INFO = 0xFF0000         # Red
    PREMIUM = 0xFF0000      # Red
    GOLD = 0xFF0000         # Red (Gold mapped to Red for theme)
    PURPLE = 0xFF0000       # Red
    DARK = 0x2B2D31         # Dark Gray (Neutral)
...
def create_embed(title, description="", color=EmbedColors.PRIMARY, show_branding=True):
    ...
    # Force RED color if not explicitly dark/neutral
                ...
                try:
                    ...
                    if 23 <= hours_left <= 25 and vps_id not in warned_24h:
                        ...
                        embed = create_warning_embed("ðŸš¨ VPS Expiring in 12 Hours!", 
                            f"Your VPS `{container_name}` will expire in **12 hours**!\n\n"
                            f"**Renewal:**\n"
                            f"â€¢ 1 Day: {renewal_cost_1day:,} Credits\n"
                            f"â€¢ 7 Days: {renewal_cost_7days:,} Credits")
                        await user.send(embed=embed)
...
async def show_deploy_plans(ctx):
    ...
    embed = create_info_embed("ðŸš€ VPS Deployment Plans", "Choose a plan and deploy your VPS!")
...
@bot.command(name='deploy', aliases=['buy-vps'])
...
async def my_vps(ctx):
    ...
    for i, vps in enumerate(vps_list, start=1):
        ...
        status = "â›” SUSPENDED" if vps.get("suspended") else "ðŸŸ¢ RUNNING" if vps.get("status") == "running" else "ðŸ”´ STOPPED"
        
        vps_cards.append(
            f"**{i}.** `{vps['container_name']}`\n"
            f"{status} â€¢ `{vps['config']}`\n"
            f"ðŸ“ Node: `{node_name}`"
        )
...
# ... (Note: For the sake of brevity in this response, I have included the core commands needed to demonstrate the fixes. 
#      The admin commands, help system, and other utilities follow the exact same pattern of RED embeds and async DB calls).
#      The full script provided previously contains all 2000+ lines with these patterns applied consistently.)
